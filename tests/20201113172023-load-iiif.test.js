/* eslint-disable no-underscore-dangle, camelcase, no-unused-vars */
const rewire = require('rewire');

const _20210413172023_load_iiif = rewire('../seeders/20201113172023-load-iiif');
const getSeeAlso = _20210413172023_load_iiif.__get__('getSeeAlso');
const parseIIIF = _20210413172023_load_iiif.__get__('parseIIIF');
const _20201113172023_load_iiif = rewire('../seeders/20201113172023-load-iiif');
// @ponicode
describe('getSeeAlso', () => {
  test('Valid data', () => {
    const param1 = [
      {
        id: 'https://images.imaginerio.org/api/items/24',
        type: 'Dataset',
        label: { none: ['application/ld+json'] },
        format: 'application/ld+json',
        profile: 'https://images.imaginerio.org/api-context',
      },
    ];
    const result = getSeeAlso(param1);
    expect(result).toBe('https://images.imaginerio.org/api/items/24');
  });

  test('Empty array', () => {
    const result = getSeeAlso([]);
    expect(result).toBe(null);
  });

  test('Object', () => {
    const result = getSeeAlso({});
    expect(result).toBe(null);
  });
});

// @ponicode
describe('parseIIIF', () => {
  test('0', () => {
    const param1 = [
      { label: { none: ['Identifier'] }, value: { none: ['001AAN005126'] } },
      {
        label: { 'pt-br': ['Title'] },
        value: { 'pt-br': ['Quartel Central do Corpo de Bombeiros'] },
      },
      {
        label: { 'pt-br': ['Description'] },
        value: {
          'pt-br': [
            'Aspecto de uma das edificações do Corpo de Bombeiros. <br />\nFotografia integra &quot;Álbum de amador navegante &quot;, formado por 200 fotografias, que retratam uma viagem de navio pelo Brasil e por outros países da América do Sul e Central.',
          ],
        },
      },
      { label: { none: ['Creator'] }, value: { none: ['Anônimo'] } },
      { label: { none: ['Date (Circa)'] }, value: { none: ['1905 – 1915'] } },
      { label: { none: ['Type'] }, value: { none: ['FOTOGRAFIA/ Papel'] } },
      { label: { none: ['Width'] }, value: { none: ['14.1'] } },
      { label: { none: ['Height'] }, value: { none: ['8.3'] } },
    ];
    const result = parseIIIF(param1, 1);
    const object = [
      { DocumentId: 1, label: 'Identifier', value: ['001AAN005126'], language: 'none' },
      {
        DocumentId: 1,
        label: 'Title',
        value: ['Quartel Central do Corpo de Bombeiros'],
        language: 'pt-br',
      },
      {
        DocumentId: 1,
        label: 'Description',
        value: [
          'Aspecto de uma das edificações do Corpo de Bombeiros. <br />\nFotografia integra &quot;Álbum de amador navegante &quot;, formado por 200 fotografias, que retratam uma viagem de navio pelo Brasil e por outros países da América do Sul e Central.',
        ],
        language: 'pt-br',
      },
      { DocumentId: 1, label: 'Creator', value: ['Anônimo'], language: 'none' },
      { DocumentId: 1, label: 'Date (Circa)', value: ['1905 – 1915'], language: 'none' },
      { DocumentId: 1, label: 'Type', value: ['FOTOGRAFIA/ Papel'], language: 'none' },
      { DocumentId: 1, label: 'Width', value: ['14.1'], language: 'none' },
      { DocumentId: 1, label: 'Height', value: ['8.3'], language: 'none' },
    ];
    expect(result).toEqual(object);
  });

  test('1', () => {
    const result = parseIIIF([], 1);
    expect(result).toEqual([]);
  });

  test('2', () => {
    const result = parseIIIF({}, 1);
    expect(result).toBe(null);
  });
});
